...
#define RESTAURA_SP(SP) __asm volatile("mov     r0, %0\n\t" ::"r"(SP));

#define SALVA_SP(SP) __asm volatile("str r0, [r1]   \n\t" ::"r1"(&SP));

#define SALVA_CONTEXTO() __asm volatile("MRS     R0,PSP	\n"         \
                                        "SUBS     R0, R0, #0x10	\n" \
                                        "STM     R0!,{R4-R7}	\n"    \
                                        "MOV     R4,R8          \n" \
                                        "MOV     R5,R9          \n" \
...
#define RESTAURA_CONTEXTO() __asm volatile(/* Restore r4-11 from new process stack */                                                \
                                           "LDM     R0!,{R4-R7}		\n"                                                                 \
                                           "MOV     R8,R4              \n"                                                           \
                                           "MOV     R9,R5              \n"                                                           \
                                           "MOV     R10,R6             \n"                                                           \
