...
uint8_t escalonador(void)
{
	...
	/* lašo comeca pela maior prioridade ate encontrar
...
/* Servišos do gerenciador de tarefas */
void TarefaSuspende(uint8_t id_tarefa)
{
	REG_ATOMICA_INICIO();
	TCB[id_tarefa].estado = ESPERA; /* tarefa Ú colocada em espera */
...
{
	REG_ATOMICA_INICIO();
	TCB[id_tarefa].estado = PRONTA; /* tarefa Ú colocada na fila de prontas */
	TrocaContexto();				/* tarefa atual solicita troca de contexto */
	REG_ATOMICA_FIM();
}
...
void TarefaEspera(tick_t qtas_marcas)
{
	if (qtas_marcas > 0) //** so valores maiores que 0 */
	{
		...
		TCB[tarefa_atual].tempo_espera = qtas_marcas; /* o contador de marcas da tarefa Ú iniciado com o valor recebido */
		TCB[tarefa_atual].estado = ESPERA;			  /* tarefa Ú colocada na fila de espera */
